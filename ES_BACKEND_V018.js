  ...
  fetch(`${fetchaddress}`, {
    ...
    .then((result) => {
      ...
      if(result.text[0].length>2){
        console.log("inside language selection function",result.text[0])
      displayMessage(result.text[0], result.urls, "BOT", null);
      }
      console.log("resutl.text came from api",result.text)
      let textindex = 0;
      console.log("richcontent")
      console.log(result.rich.richContent)
          ...
          content.options.forEach((option) => {
            ...
            chipButton.addEventListener("click", () => {
              ...
              if (content.type === "chips") {
                ...
                console.log("inside chips dialogflow data",dialogflowdata.text)
                console.log("length of text",dialogflowdata.text.trim().length)
                let t = 'Start Again'
                ...
                fetch(`${newfetchaddress}`, {
                  ...
                  .then((result) => {
                    ...
                    for (const content of result.rich.richContent) {
                      ...
                      displayMessage(result.text[textindex], result.urls, "BOT", [
                        content,
                      ]);
                    }}
